	      	...
	      	<div class="breadcrumb flat">
    			...
    			{!! Form::open(array('url'=>'apcsquestion/uploadFiles', 'data-parsley-validate' => '', 'method'=>'POST', 'enctype' => 'multipart/form-data', 'files'=>true)) !!}
				      ...
				      <tr style="background-color: #4CAF50; color: white;">
				        ...
				        <th colspan="2" style="width:15%;">Sub-Answer</th>
				        <th colspan="2" style="width:15%;">Answer</th>
				        <th style="width:7%;">Correct Answer</th>
				        <th style="width:7%;">Related Refference</th>
				      ...
				      <tr>
				        ...
				        <td rowspan="4">
				        @if (count($questions->original_filename) === 1)
				        <img class="file" src="/uploads/{{$questions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$questions->original_filename}}"><br><center>{{$questions->limg}}</center></img>
				        @else
				        @endif
				        </td>
				        ...
				        <td rowspan="4" id="but"><button>Approve<br>Question</button>
				        <br><br><button type="button" style="width:200%;" title="Question Preview" data-toggle="modal" data-target="#yourModal{{$questions->id}}">Preview</button>
				        </td>
				        <td rowspan="4"><center><a href="{{ route('apcsquestion.edit', $questions->id) }}" >
				        	{{ HTML::image('images/edit.png', 'a Edit', array( 'width' => '20', 'title' => 'Edit')) }}</a> </center><br></td>

				        	<td rowspan="4">{!! Form::open(['route' => ['apcsquestion.destroy', $questions->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
				  ...
				  @foreach($question as $questions)    
				    <div class="modal fade" id="yourModal{{$questions->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				      <div class="modal-dialog" role="document">
				        <div class="modal-content">
				          ...
				          @if (count($questions->original_filename) === 1)
				          <center><img class="file" src="/uploads/{{$questions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$questions->original_filename}}"></center>
				          <br><center>{{$questions->limg}}</center></a><br>
				          @else   
						  @endif
							...
							@if (count($questions->ii) === 1)
				            <br>II. {{$questions->ii}}
				            @else   
							@endif

							@if (count($questions->iii) === 1)
				            <br>III. {{$questions->iii}}
				            @else   
							@endif

							@if (count($questions->iv) === 1)
				            <br>IV. {{$questions->iv}}
				            @else   
							@endif

						    <br><br><input type="radio" name="level" required="" value="a"> {{$questions->a}}
							<br><input type="radio" name="level" required="" value="b"> {{$questions->b}}
							<br><input type="radio" name="level" required="" value="c"> {{$questions->c}}
							<br><input type="radio" name="level" required="" value="d"> {{$questions->d}}
