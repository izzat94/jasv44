...
// $_SESSION['result'] = '<div id="result"></div>';
?>
				    ...
				    @foreach($announce as $announces)
				      <tr>
				        ...
				        <td id="but" align="right">
				        	...
				        	{{ HTML::image('images/edit.png', 'a Edit', array( 'width' => '20', 'title' => 'Edit')) }}</a><br></td>

				        	<td>{!! Form::open(['route' => ['announce.destroy', $announces->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
...
var rows = [].slice.call(document.getElementById('myTable').rows);
result.innerHTML  = rows.map(function(row1, row2){
return row1.cells.length;
}).join('<br>');
	</script>
</body>
</html>