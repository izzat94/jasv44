		...
		<div>
			...
			<div class="navbar templatemo-nav" id="navbar">
	      	...
	      	<div class="breadcrumb flat">
    			...
    			{!! Form::open(array('url'=>'ietsquestion/uploadFiles', 'data-parsley-validate' => '', 'method'=>'POST', 'enctype' => 'multipart/form-data', 'files'=>true)) !!}
					  ...
					  {!! Form::close() !!}       
				  		...
				  		<table id="list" class="table" style="width:90%;" align="center">
				      ...
				      <tr style="background-color: #4CAF50; color: white;">
				        ...
				        <th style="width:10%;"><center>Uploaded By</center></th>
				      </tr>
				      <tbody>
				      ...
				      <tr>
				        ...
				        <td rowspan="4">
				        @if (count($ietsquestions->original_filename) === 1)
				        <img class="file" src="/uploads/{{$ietsquestions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$ietsquestions->original_filename}}"><br><center>{{$ietsquestions->limg}}</center></img>
				        @else
				        @endif
				        </td>
				        ...
				        <td rowspan="4">{{$ietsquestions->ietsrelated->ietsrelated}}</td>
				        <td rowspan="4">{{$ietsquestions->level}}</td>
				        <td rowspan="4" id="but" align="right"><button>Approve<br>Question</button>
				        <br><br><button type="button" style="width:200%;" title="Question Preview" data-toggle="modal" data-target="#yourModal{{$ietsquestions->id}}">Preview</button>
				        </td>
				        <td rowspan="4"><center><a href="{{ route('ietsquestion.edit', $ietsquestions->id) }}" >
				        	{{ HTML::image('images/edit.png', 'a Edit', array( 'width' => '20', 'title' => 'Edit')) }}</a> </center><br></td>

				        	<td rowspan="4">{!! Form::open(['route' => ['ietsquestion.destroy', $ietsquestions->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
				  ...
				  @foreach($ietsquestion as $ietsquestions)    
				    <div class="modal fade" id="yourModal{{$ietsquestions->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				      <div class="modal-dialog" role="document">
				        <div class="modal-content">
				          ...
				          @if (count($ietsquestions->original_filename) === 1)
				          <center><img class="file" src="/uploads/{{$ietsquestions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$ietsquestions->original_filename}}"></center>
				          <br><center>{{$ietsquestions->limg}}</center></a><br>
				          @else   
						  @endif

				          

				            {{$ietsquestions->id}}. {{$ietsquestions->question}}

				           	@if (count($ietsquestions->i) === 1)
				            <br><br>I. {{$ietsquestions->i}}
				            @else   
							@endif

							@if (count($ietsquestions->ii) === 1)
				            <br>II. {{$ietsquestions->ii}}
				            @else   
							@endif

							@if (count($ietsquestions->iii) === 1)
				            <br>III. {{$ietsquestions->iii}}
				            @else   
							@endif

							@if (count($ietsquestions->iv) === 1)
				            <br>IV. {{$ietsquestions->iv}}
				            @else   
							@endif

						    <br><br><input type="radio" name="level" required="" value="a"> {{$ietsquestions->a}}
							<br><input type="radio" name="level" required="" value="b"> {{$ietsquestions->b}}
							<br><input type="radio" name="level" required="" value="c"> {{$ietsquestions->c}}
							<br><input type="radio" name="level" required="" value="d"> {{$ietsquestions->d}}
